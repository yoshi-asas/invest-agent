...
with tab_portfolio:
    ...
    try:
        ...
        if "connections" in st.secrets and "gsheets" in st.secrets.connections:
            ...
            # 日本語版Googleスプレッドシートのデフォルトは「シート1」、英語版は「Sheet1」
            ws_name = "シート1"
            try:
            ...
            if df is not None and not df.empty and len(df.columns) > 0:
                st.info("💡 以下の表の行を選択してパソコンの「Delete」キーを押すか、右側のゴミ箱で銘柄を削除できます。数値を直接クリックして書き換えることも可能です。")
                edited_df = st.data_editor(df, num_rows="dynamic", use_container_width=True)
                if st.button("🔄 表の変更（削除や修正）をクラウドに保存", type="primary"):
                    conn.update(worksheet=ws_name, data=edited_df)
                    st.success("データベースの変更を保存しました！")
                    st.rerun()
            else:
                st.info("スプレッドシートにデータが登録されていません。（スプレッドシートの1行目に「保有銘柄」「企業名」「保有株数」「取得単価」という見出しを作成してください）")
            ...
            with st.form("add_stock_form"):
                ...
                submitted = st.form_submit_button("💼 データベース（スプレッドシート）に登録する")
                if submitted:
                    if new_ticker:
                        ...
                        st.success(f"{t_sym} をクラウドデータベースに登録しました！")
                        st.rerun()
                    else:
